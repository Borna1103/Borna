...
export default function Classes() {
    return (
        <div className='rounded-2xl borderp-6 border-zinc-100 border-zinc-700/40 bg-gray-800 p-6 pl-8 overflow-y-auto no-scrollbar classes'>
               ...
               {data.map((course) => (
                <li key={course.id} className="flex gap-4">
                    ...
                    <div className="flex flex-auto flex-wrap gap-x-2">
                        <h2 className="w-full flex-none text-sm font-medium text-zinc-900 dark:text-zinc-100">{course.name}</h2>
                        <h2 className="text-xs text-zinc-500 dark:text-zinc-400">{course.skills}</h2>
                        <time className="ml-auto text-xs text-zinc-400 dark:text-zinc-500">{course.time}</time>
                    </div>
                </li>
               ))} 
            </ol>
